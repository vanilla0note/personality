<?php
    ...
    function answer_edit_view_colony($mood_id){
        ...
        switch($colony_read_title){
            ...
            case "AREA":
                ...
                for ( $i=0 ; $i<count($colony_content_arr) ; $i++ ){
                    if( substr($mood_id , 0 , 1) == substr($colony_content_arr[$i] , 5 , 1) )  
                        $colony = "true" ; 
                }
                break;
            ...
            case "DEP":
                ...
                $sql_temp = "SELECT * FROM `mood` WHERE `mood_id` = '{$mood_id}'";
                $result_temp = mysqli_query($link, $sql_temp) ;
                while( $row_temp = mysqli_fetch_assoc($result_temp) ) 
                    $rows_temp[] = $row_temp;
                $staff_dep   = $rows_temp[0]["staff_dep"] ;   //讀取權限類別

                for ( $i=0 ; $i<count($colony_content_arr) ; $i++ ){
                    if( $staff_dep == $colony_content_arr[$i] )  
                        $colony = "true" ; 
                }
                break;
